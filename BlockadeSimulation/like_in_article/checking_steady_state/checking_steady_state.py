...
def simulate():
    results = []
    for drive in drives:
        result = BlockadeSimulation.Simulations._blockade_step2_start_undisplaced(N, U=0.075 * kappa_total, Lambda_3=drive * kappa_total, times=times, dL_1=0.01)
        mean_n = expect(num(N), result.states)
        results.append(result)
    np.savez(
        file='checking_steady_state_data.npz',
        drives=drives,
        results=results)
def plot_occupation():
    file = np.load('checking_steady_state_data.npz', allow_pickle=True)
    drives = file['drives']
